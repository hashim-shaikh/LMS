...
import 'package:chandrakalm/model/InstituteDetailsModel.dart';
...
class InstituteCourseListItem extends StatelessWidget {
  final Course courseDetail;

  InstituteCourseListItem(this.courseDetail);
        ...
        onTap: () {
          ...
          Navigator.of(context).pushNamed("/courseDetails",
              arguments: DataSend(details.userId, isPurchased,
                  details.id as int, details.categoryId, details.type));
        },
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    bool isPurchased = Provider.of<CoursesProvider>(context)
        .isPurchased(courseDetail.id as int);

    String rating = "0.0";

    Provider.of<CoursesProvider>(context).allCourses!.forEach((course) {
      if (course.id.toString() == courseDetail.id.toString()) {
        rating = getRating(course.review);
      }
    });
